...
def main():
 ...
 src=root/"results/suppfig2_fair_matched_candidate_v152";out=root/"figures/suppfig2_v19_11_antialiased_maps";out.mkdir(parents=True,exist_ok=True)
 dat={}
 ...
 for organ,y0 in [("Brain",.790),("Kidney",.565)]:
  ...
  for x0,(name,col) in zip(xs,MAPS):
   ...
   # DISPLAY ONLY: the statistic remains the same spatial-bin median.
   # Smaller circular marks + high-DPI output reduce the block/pixel appearance
   # without smoothing, interpolating, or changing any bin value.
   im=ax.scatter(
    X,Y,c=V,
    s=.52 if organ=="Brain" else .22,
    marker="o",cmap="coolwarm",norm=mapnorm,
    linewidths=0,edgecolors="none",alpha=.82,
 ...
 fig.savefig(out/"suppfig2_v19_11_antialiased_maps.png",dpi=700,facecolor="white")
 fig.savefig(out/"suppfig2_v19_11_antialiased_maps.pdf",facecolor="white")
