...
V2 = ROOT / "results" / "Fig5_Disease_Trajectories" / "gw_geodesic_v2"
OUT = ROOT / "results" / "Fig5_Disease_Trajectories" / "gw_multistart_v3"
...
def load_v2_index():
    ...
    if len(traj) != 7:
        raise RuntimeError(
            f"Expected 7 K={K} rows in {traj_path.name}; found {len(traj)}"
        )
