...
export SUTRA_LEVEL0_WORKERS="${SUTRA_LEVEL0_WORKERS:-3}"
export SUTRA_HIERARCHY_WORKERS="${SUTRA_HIERARCHY_WORKERS:-3}"
export SUTRA_WORKERS="${SUTRA_WORKERS:-3}"
...
mod=Path(sutra.__file__).resolve()
print("SUTRA ownership preflight")
print(" python:", sys.executable)
print(" sutra:", mod)
if src not in mod.parents:
    raise SystemExit(f"ERROR: sutra import is outside SUTRA/src: {mod}")
...
pids=()
for S in "${SAMPLES[@]}"; do
  "$PY" -m sutra.cli.tranche2_2b_prepare \
    --project-root "$ROOT" \
    --sample "$S" \
    --max-cells 0 \
...
run_stage level0_v070 env SUTRA_LEVEL0_WORKERS="${SUTRA_LEVEL0_WORKERS:-3}" ./run_level0_step3_v070.sh
